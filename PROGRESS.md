...
- 2026-09-12 — T20.1 — migration 20260912000000_connectors (ConnectorConfig with globally unique credentialRef, ConnectorCredentialRotation, ConnectorTestRun, three enums) — additive only, applied; `connector.manage` added for GROUP_OWNER, PRACTICE_PARTNER, IT_ADMIN — not yet tested
- 2026-09-12 — T20.2 — src/lib/connectors.ts: env:/vault: reference-only guard (platform secrets refused), reference unique + never reusable after rotation, runtime-environment guard (secret never read on mismatch), probe seam with scrubSecret (withholds message if any 8-char fragment survives), rotation touching one row, ACTIVE requires PASSED test on current credential version; ConnectorError mapped in api.ts — tsc clean — not yet tested
- 2026-09-12 — T20.3 — tests/t20-connectors.ts run BEFORE routes: 48/48 — the eight INT01 fields stored and the secret absent from the row; a pasted secret and env:DATABASE_URL refused; a probe that echoes the token (raw + base64) is redacted and one that throws a FRAGMENT is withheld, with no 8-char fragment anywhere in test runs/audit/config (control: the probe did receive the secret); PROD connector refused in a DEV runtime and DEV in PROD with the probe never called; Company rotation leaves the Associates row byte-identical and its live test still PASSES; retired reference never reusable — PASS
- 2026-09-12 — T20.4 — routes GET|POST /api/connectors, GET|POST /api/connectors/[connectorId] (test | rotate | status; practice read from the STORED row), /connectors screen + loading.tsx (five states, references only), Practice link, dev:walk entry. tsc + eslint clean; unauthenticated GET/POST → 401 with the standard envelope. dev:walk now checks the Connectors BODY, not just the h1 — "Connectors" is also the permission/error title, so the h1 alone reported OK while the screen showed an error. That check caught it: the dev server started 2026-09-11 held a stale Prisma client (`prisma.connectorConfig` undefined — the post-generate restart note above, third time). Server restarted — walk re-run: "Connectors screen (INT01): list and create form rendered", every linked destination resolves — PASS

- 2026-09-12 — T20 — COMPLETE. Connector configuration (INT01; INT02 is R1) — tested against the INT01 acceptance line via `npm run test:t20` (48/48): all eight fields stored with a credential REFERENCE and no secret; a connection test discloses no secret (echoed token redacted, a leaked fragment withholds the whole message, nothing in test runs / audit / config — with a control proving the probe received it); dev and production credentials separate (a reference is unique across practices AND environments, a wrong-environment connector is refused without the secret ever being read); rotating Company's token leaves the Associates connector byte-identical and still passing a live test — PASS.
  Regression run this time: T04 37/37 (matrix changed), T19 94/94, dev:walk green. The other suites were NOT re-run — owner allowed skipping low-risk tests for the 2026-09-13 delivery. T25 must run the full suite.
  NO live connector ships: the default probe proves the reference resolves and says in words it did not contact the remote service; `setVaultResolver` is an unattached seam (no vault chosen — follows PRD §46). Attaching real provider probes and a secret vault is owner work before go-live.
  Next: T21 — Controlled import (MIG01-03, PRD §43).
